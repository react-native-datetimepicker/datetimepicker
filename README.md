...
> To localize the calendar days (avoid mixed language for month and day names): If you use a library like [i18n](https://github.com/i18next/react-i18next) or [react-localize-redux](https://github.com/ryandrewjohnson/react-localize-redux) to manage your translations, it is sufficient to add your target languages in the `project.pbxproj` as described in the Apple Documentation - but you are not required to add any localization keys (like the days of the week). Apple / Xcode will automatically distribute and apply the localized keys as long as the target language is contained in `project.pbxproj`.
